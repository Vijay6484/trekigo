...
interface CouponFormData {
    ...
    active: boolean;
    expiryDate: string;
    accommodationType?: number; // <-- Add this line
}
...
const defaultCoupon: CouponFormData = {
    ...
    active: true,
    expiryDate: "",
    accommodationType: 0, // <-- Add this line
};
...
const Coupons: React.FC = () => {
    ...
    const [editingCoupon, setEditingCoupon] = useState<CouponFormData | null>(
        null,
    );
    ...
    const fetchAccommodation = useCallback(async () => {
        try {
            const response = await axios.get(
                `${API_BASE_URL}/properties/accommodations`,
            );
            ...
            if (Array.isArray(accommodations)) {
                ...
                console.error(
                    "No accommodations found or invalid response shape",
                );
                ...
                setCoupons(
                    data.data.map((c: any) => ({
                        ...
                        minAmount: c.minAmount
                            ? parseFloat(c.minAmount)
                            : undefined,
                        maxDiscount: c.maxDiscount
                            ? parseFloat(c.maxDiscount)
                            : undefined,
                        usageLimit: c.usageLimit
                            ? parseInt(c.usageLimit)
                            : undefined,
                        ...
                        accommodationType: c.accommodationType || 0, // <-- Add this line
                    })),
                );
            } else {
                setCoupons([]);
    ...
    const handleToggleStatus = async (id: number) => {
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/coupons/${id}/toggle`,
                {
                    method: "PATCH",
            ...
            if (response.ok && data.success) {
                showMessage(
                    data.message || "Coupon status updated successfully",
                    "success",
                );
                await fetchCoupons();
            } else {
                throw new Error(
                    data.message || "Failed to update coupon status",
                );
            }
        } catch (error) {
            showMessage(
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/coupons`, {
                ...
                body: JSON.stringify({
                    ...
                    accommodationType: newCoupon.accommodationType || 0,
                }),
            });
    ...
    const openEditModal = (coupon: Coupon) => {
        setEditingCoupon({
            ...
            expiryDate: coupon.expiryDate
                ...
                : "",
            accommodationType: coupon.accommodationType || 0, // <-- Add this line
        });
        ...
        switch (filter) {
            case "active":
                return filtered.filter(
                    (coupon) => coupon.active && !isExpired(coupon.expiryDate),
                );
            case "inactive":
                return filtered.filter((coupon) => !coupon.active);
            case "expired":
                return filtered.filter((coupon) =>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center space-x-1">
                                                    ...
                                                    <span
                                                        className={`text-sm ${expired ? "text-red-600 font-medium" : "text-gray-500"}`}
                                                    >
                                                        {formatDate(
                                                            coupon.expiryDate,
                                                        )}
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                        ...
                                        <div className="mt-4">
                                            <div className="space-y-4">
                                                ...
                                                <div>
                                                    <label
                                                        htmlFor="edit-accommodationType"
                                                        className="block text-sm font-medium text-gray-700"
                                                    >
                                                        Accommodation
                                                    </label>
                                                    <select
                                                        ...
                                                        value={
                                                            editingCoupon.accommodationType
                                                        }
                                                        onChange={(e) =>
                                                            setEditingCoupon({
                                                                ...editingCoupon,
