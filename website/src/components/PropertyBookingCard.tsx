...
import { fetchPropertyCalendar } from "@/lib/api";
import {
  checkoutHref,
  formatINR,
  formatShortDate,
  ...
  nightsBetween,
  parsePrice,
  stayTotalFromCalendar,
  suggestedStay,
  type RoomGuests,
} from "@/lib/booking";
import type { CalendarDay, WebsiteProperty } from "@/lib/types";

export function PropertyBookingCard({ property }: { property: WebsiteProperty }) {
  ...
  useEffect(() => {
    fetchPropertyCalendar(property.id).then(setCalendar).catch(() => setCalendar([]));
  }, [property.id]);

  const nights = checkIn && checkOut ? nightsBetween(checkIn, checkOut) : 0;
  const guests = guestSummary(rooms);
  const nightly = property.nightly || parsePrice(property.price);
  const blockedKeys = calendar.filter((day) => day.blocked).map((day) => day.date);
  const estimate = stayTotalFromCalendar(nightly, checkIn, checkOut, calendar, rooms.length);
