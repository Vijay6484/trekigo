...
cron.schedule('0 2 * * *', async () => {
  try {
    const [result] = await pool.execute(
      `DELETE FROM blocked_dates WHERE date(blocked_date) < date(?)`,
      [todayKey()]
    );
    ...
    for (const date of dates) {
      const [existingRows] = await pool.execute(
        `SELECT id FROM blocked_dates
         WHERE accommodation_id = ? AND DATE(blocked_date) = DATE(?)
         LIMIT 1`,
      ...
      if (existingRows.length) {
        await pool.execute(
          `UPDATE blocked_dates
           SET reason=?, rooms=?, adult_price=?, child_price=?, updated_at=?
           WHERE id=?`,
          [
            reason,
            room_number === null || room_number === undefined
              ? null
              : String(room_number),
            adult_price,
            child_price,
...
router.get('/accommodations', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT id, name, type, rooms, price, adult_price, child_price FROM accommodations');
    res.json(rows);
  } catch (error) {
