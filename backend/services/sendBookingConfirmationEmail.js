/**
 * Booking confirmation HTML email (extracted from routes/bookings.js).
 */
require("dotenv").config();
const nodemailer = require("nodemailer");

function buildBookingConfirmationBodies(params) {
  const {
    email,
...
<body class="body"
  ...
  return { html, html_villa };
}

async function sendPdfEmail(params) {
  const { email, ownerEmail, accommodation_type } = params;
  ...
  if (accommodation_type === "Villa") {
    emailBody = html_villa
  }
  const mailOptions2 = {
    from: "bookings@nirwanastays.com",
    to: "anil.patharde@foxaircomm.com",
    subject: "NirwanaStays Resort Booking",
    html: emailBody, // Make sure HTML variable is defined
  };
  ...
  try {
    let info;
    const skipInternalCopy =
      process.env.BOOKING_EMAIL_SKIP_INTERNAL_COPY === "1" ||
      process.env.BOOKING_EMAIL_SKIP_INTERNAL_COPY === "true";
...
module.exports.buildBookingConfirmationBodies = buildBookingConfirmationBodies;
