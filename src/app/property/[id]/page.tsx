...
export default async function PropertyDetailsPage({
  ...
  const [property, allProperties] = await Promise.all([fetchProperty(id), fetchProperties()]);
  if (!property) notFound();
  const related = allProperties.filter((item) => item.id !== property.id).slice(0, 4);
  const images = property.images?.length ? property.images : galleryImages;
  const amenityItems = property.amenities?.length ? property.amenities : fallbackAmenities;

  return (
    <div className="pt-16">
      <PropertyHeader />
      <main className="mx-auto max-w-[1280px] px-container-margin-mobile py-6 pb-28 md:px-container-margin-desktop">
        <PhotoGallery images={images} alt={property.name} />
        ...
        <div className="grid gap-10 lg:grid-cols-12">
          ...
          <section className="rounded-xl border border-outline-variant bg-surface p-5">
            ...
            <ul className="space-y-3 text-sm text-on-surface-variant">
              ...
              <li className="flex gap-2">
                <Icon name="check_circle" />
                {property.rooms || 1} room{(property.rooms || 1) === 1 ? "" : "s"} · up to {property.capacity || 2} guests
              </li>
            </ul>
          </section>
          ...
          <section className="rounded-xl border border-outline-variant bg-surface p-5">
            ...
            <p className="text-sm leading-7 text-on-surface-variant">
              {property.description ||
                "A quiet hill stay with open views, a private pool, and interiors made for long weekends."}
            </p>
          </section>
